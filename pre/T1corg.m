function T1corg(path, name, work, startname)
    ...
    Tc = sprintf('mrtransform %s/5tt.mif -linear %s/T1_to_DWI_mrtrix.txt -target %s/mean_b0.nii.gz %s/5tt_in_dwi.mif -force', ...
        output, output, b0_path, output);
    system(Tc);

    Tc = sprintf('mrtransform %s/gmwmSeed.mif -linear %s/T1_to_DWI_mrtrix.txt -target %s/mean_b0.nii.gz %s/gmwmSeed_in_dwi.mif -force', ...
        output, output, b0_path, output);
    system(Tc);
