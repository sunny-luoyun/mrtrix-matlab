function fa(name, work)
    ...
    outputresultpath = fullfile(work, 'Results', 'tensormetric','FA'); 
    swappath = fullfile(work,'dwi_coreg',name);
    mkdir(outputresultpath)
    % 计算指标
    cmd = sprintf('tensor2metric %s/dt.mif -fa %s/FA.mif -force', ... 
        inputdtpath, inputdtpath);
    system(cmd);
    % 配准
    cmd = sprintf('mrtransform %s/FA.mif -linear %s/dwi_to_MNI_mrtrix.txt -template %s %s/FA_coreg.mif -force', ... 
        inputdtpath, swappath, template_path, inputdtpath);
    system(cmd);
    % 转换
    cmd = sprintf('mrconvert %s/FA_coreg.mif %s/%s_FAmap.nii -force', ... 
