function rk(name, work)
    ...
    outputresultpath = fullfile(work, 'Results', 'tensormetric','RK'); 
    swappath = fullfile(work,'dwi_coreg',name);
    mkdir(outputresultpath)
    % 计算指标
    cmd = sprintf('tensor2metric %s/dkt.mif -rk %s/RK.mif -force', ... 
        inputdtpath, inputdtpath);
    system(cmd);
    % 配准
    cmd = sprintf('mrtransform %s/RK.mif -linear %s/dwi_to_MNI_mrtrix.txt -template %s %s/RK_coreg.mif -force', ... 
        inputdtpath, swappath, template_path, inputdtpath);
    system(cmd);
    % 转换
    cmd = sprintf('mrconvert %s/RK_coreg.mif %s/%s_RKmap.nii -force', ... 
