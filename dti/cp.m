function cp(name, work)
    ...
    outputresultpath = fullfile(work, 'Results', 'tensormetric','CP'); 
    swappath = fullfile(work,'dwi_coreg',name);
    mkdir(outputresultpath)
    % 计算指标
    cmd = sprintf('tensor2metric %s/dt.mif -cp %s/CP.mif -force', ... 
        inputdtpath, inputdtpath);
    system(cmd);
    % 配准
    cmd = sprintf('mrtransform %s/CP.mif -linear %s/dwi_to_MNI_mrtrix.txt -template %s %s/CP_coreg.mif -force', ... 
        inputdtpath, swappath, template_path, inputdtpath);
    system(cmd);
    % 转换
    cmd = sprintf('mrconvert %s/CP_coreg.mif %s/%s_CPmap.nii -force', ... 
