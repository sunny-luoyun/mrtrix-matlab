function cs(name, work)
    ...
    outputresultpath = fullfile(work, 'Results', 'tensormetric','CS'); 
    swappath = fullfile(work,'dwi_coreg',name);
    mkdir(outputresultpath)
    % 计算指标
    cmd = sprintf('tensor2metric %s/dt.mif -cs %s/CS.mif -force', ... 
        inputdtpath, inputdtpath);
    system(cmd);
    % 配准
    cmd = sprintf('mrtransform %s/CS.mif -linear %s/dwi_to_MNI_mrtrix.txt -template %s %s/CS_coreg.mif -force', ... 
        inputdtpath, swappath, template_path, inputdtpath);
    system(cmd);
    % 转换
    cmd = sprintf('mrconvert %s/CS_coreg.mif %s/%s_CSmap.nii -force', ... 
