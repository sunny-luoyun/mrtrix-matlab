function ak(name, work)
    ...
    outputresultpath = fullfile(work, 'Results', 'tensormetric','AK'); 
    swappath = fullfile(work,'dwi_coreg',name);
    mkdir(outputresultpath)
    % 计算指标
    cmd = sprintf('tensor2metric %s/dkt.mif -ak %s/AK.mif -force', ... 
        inputdtpath, inputdtpath);
    system(cmd);
    % 配准
    cmd = sprintf('mrtransform %s/AK.mif -linear %s/dwi_to_MNI_mrtrix.txt -template %s %s/AK_coreg.mif -force', ... 
        inputdtpath, swappath, template_path, inputdtpath);
    system(cmd);
    % 转换
    cmd = sprintf('mrconvert %s/AK_coreg.mif %s/%s_AKmap.nii -force', ... 
