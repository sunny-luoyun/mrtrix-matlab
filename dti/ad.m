function ad(name, work)
    ...
    outputresultpath = fullfile(work, 'Results', 'tensormetric','AD'); 
    swappath = fullfile(work,'dwi_coreg',name);
    mkdir(outputresultpath)
    % 计算指标
    cmd = sprintf('tensor2metric %s/dt.mif -ad %s/AD.mif -force', ... 
        inputdtpath, inputdtpath);
    system(cmd);
    % 配准
    cmd = sprintf('mrtransform %s/AD.mif -linear %s/dwi_to_MNI_mrtrix.txt -template %s %s/AD_coreg.mif -force', ... 
        inputdtpath, swappath, template_path, inputdtpath);
    system(cmd);
    % 转换
    cmd = sprintf('mrconvert %s/AD_coreg.mif %s/%s_ADmap.nii -force', ... 
