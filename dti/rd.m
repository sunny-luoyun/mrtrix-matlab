function rd(name, work)
    ...
    outputresultpath = fullfile(work, 'Results', 'tensormetric','RD'); 
    swappath = fullfile(work,'dwi_coreg',name);
    mkdir(outputresultpath)
    % 计算指标
    cmd = sprintf('tensor2metric %s/dt.mif -rd %s/RD.mif -force', ... 
        inputdtpath, inputdtpath);
    system(cmd);
    % 配准
    cmd = sprintf('mrtransform %s/RD.mif -linear %s/dwi_to_MNI_mrtrix.txt -template %s %s/RD_coreg.mif -force', ... 
        inputdtpath, swappath, template_path, inputdtpath);
    system(cmd);
    % 转换
    cmd = sprintf('mrconvert %s/RD_coreg.mif %s/%s_RDmap.nii -force', ... 
